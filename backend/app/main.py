...
async def _seed_default_homepage_release(db):
    ...
    builtin_selected = (
        ...
        and runtime.get("active_release_id") == DEFAULT_HOMEPAGE_RELEASE_ID
    )
    desired_status = (
        HomepageReleaseStatus.ACTIVE
        if builtin_selected
    ...
    if existing_release:
        existing_release.status = desired_status
        if builtin_selected:
            if existing_release.activated_at is None:
                existing_release.activated_at = datetime.utcnow()
        else:
            existing_release.activated_at = None
        if builtin_selected:
            await db.execute(
                update(HomepageRelease)
                .where(
                    HomepageRelease.id != release_uuid,
...
async def health_check():
    """健康检查 — 返回服务状态"""
    return {"status": "ok", "service": "georank", "version": PRODUCT_VERSION}

...
(window.GEOrank?.PageLifecycle?.run?.bind(window.GEOrank.PageLifecycle)
    ...
    async function getBrowserDirectConfig() {
        ...
        const allowed = new Map((policy.allowed_byok_providers || []).map(provider => [
            String(provider.key || '').toLowerCase(),
            provider,
        ]));
        const provider = String(config.provider || 'custom').toLowerCase();
        const preset = allowed.get(provider);
        if (!preset) {
            updateDirectStatus('当前本地 API Key 的供应商不在后台允许范围内；已使用本地规则生成。', 'warn');
            return null;
        }
        try {
            if (new URL(config.baseUrl).origin !== new URL(preset.base_url).origin) {
                updateDirectStatus('本地 Base URL 与后台供应商地址不一致；已拒绝发送 API Key。', 'warn');
                return null;
            }
        } catch (_) {
            updateDirectStatus('本地 Base URL 无效；已拒绝发送 API Key。', 'warn');
            return null;
        }

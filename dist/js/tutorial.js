...
(window.GEOrank?.PageLifecycle?.run?.bind(window.GEOrank.PageLifecycle)
    ...
    function renderEmptyState(message) {
        ...
        setDocumentMeta('GEO Wiki 知识库 - GEOrank', 'GEO 生成式引擎优化知识库 — 科普教程、最佳实践与技术文档。');
    ...
    function renderArticle(detail) {
        ...
        setDocumentMeta(
            `${detail.title || 'GEO Wiki 知识库'} - GEOrank`,
    ...
    function buildChannelStepCopy(category, items) {
        ...
        return {
            primary: `这个章节收录 ${items.length} 篇围绕「${category}」展开的 Wiki 文章，按照执行场景组织，适合从基础理解一路读到落地模板。`,
            secondary: latest
    ...
    function renderChannelHome() {
        ...
        if (elements.title) elements.title.textContent = 'GEO Wiki 知识库';
        if (elements.overview) {
            elements.overview.textContent = '这里汇总当前频道下全部已发布的 GEO Wiki 教程，覆盖认知、AI 原理、内容优化、页面技术、策略执行、评估治理与案例拆解，适合按章节连续阅读，也适合按主题跳读。';

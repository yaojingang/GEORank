...
export function AdminSettings({token}: AdminSettingsProps) {
  ...
  const loadSettings = useCallback(async () => {
    ...
    setError('');
    try {
      const [settingsPayload, llmPayload, channelsPayload, modulesPayload, policyPayload] = await Promise.all([
        getAdminSettings(token),
        getAdminLLMProviders(token),
        getAdminSolutionChannels(token),
        getAdminFrontendModules(token),
        getAdminApiPolicy(token)
      ]);
      setSettings(settingsPayload);
      const nextState: FormState = {};
      Object.entries(settingsPayload).forEach(([key, item]) => {
        nextState[key] = toDisplayValue(item.value);
      });
      setFormState(nextState);
      setLlmProviders(llmPayload);
      setSolutionChannels(channelsPayload);
      setFrontendModules(modulesPayload);
      setApiPolicy(policyPayload);
      setApiPolicyDraft(JSON.stringify(policyPayload.policy || {}, null, 2));
    ...
    <main className="admin-page">
      <section className="admin-page__hero">
        ...
        <div className="admin-detail-list__actions">
          ...
          <button className="admin-button admin-button--ghost" onClick={loadSettings} type="button">
            {t('refresh')}
          </button>
